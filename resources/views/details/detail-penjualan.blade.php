...
@section('title', 'Otoparts - Detail Penjualan')
        ...
        <div class="col-12">
          <div class="card mb-2">
            <div class="card-header pb-0">
                ...
                <button class="btn btn-secondary" onclick="window.location.href='{{ url('/penjualan') }}'">Kembali</button>
            </div>
            <div class="table-responsive pb-0">
                <table class="table align-items-center mb-0">
                    <tr>
                        <td class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 ps-3">Id Penjualan</td>
                        <td class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 ps-3">{{ $sales->id_penjualan}}</td>
                    </tr>
                    <tr>
                        <td class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 ps-3">Tanggal</td>
                        <td class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 ps-3">{{ $sales->tanggal}}</td>
                    </tr>
                    <tr>
                        <td class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 ps-3">Total</td>
                        <td class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 ps-3">{{ $sales->total}}</td>
                    </tr>
                    <tr>
                        <td class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 ps-3">Kasir</td>
                        <td class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 ps-3">{{ $sales->id_user}}</td>
                    </tr>
                </table>
            </div>
          </div>  
                  ...
                  <tbody>
                    <tr>                      
                      ...
                      <td class="text-center">
                        <button class="btn btn-primary" onclick="openModalEdit({{ $sales->id_penjualan }}, {{ $row->id_barang }}, {{ $row->jumlah }}, {{ $row->subtotal}})">Ubah</button>
                        <form action="{{ url('/delete-details') . '/' . $row->id_penjualan . '/' . $row->id_barang }}" method="POST" style="display:inline;">
                          @csrf
                          @method('DELETE')
    ...
    <div class="modal-background" id="addModal">
      <div class="modal-content">
          ...
          <div class="modal-body">
              <form method="post" action="/add-details">
                @csrf
                <input type="hidden" name="id_penjualan" value="{{ $sales->id_penjualan }}">
                <div class="form-group">
                    ...
                    <select name="barang" id="barang" class="form-control" onchange="updateSubtotal()">
                        ...
                        @foreach ($products as $row)
                            <option value="{{ $row->id_barang }}" data-harga="{{ $row->harga_jual }}">{{ $row->nama_barang }}</option>
                        @endforeach
                    </select>
                </div>                
                <div class="form-group">
